...
function ResponseSubmissions() {
  ...
  const supabaseServiceKey =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtmenNwdHljb25kcnJmeGpkeGZ4Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcxNzY2NTAwMSwiZXhwIjoyMDMzMjQxMDAxfQ.W_eZV51h1uwVpjFuel2E5N5Ivs3we3XwEDQvUJrZ1XY";
  ...
  async function getLeads(a, b) {
    const query = supabase2
      .from("responses")
      .select(
        "id,created_at,name,phone,appno,our_student,category,phone,total,link,email"
      );
    setSelected([]);
  ...
  async function handleClaim(a, b, c) {
    ...
    if (data) {
      ...
      setLeads((prevItems) =>
        prevItems.map((item) =>
          item.id === b ? { ...item, our_student: a } : item
        )
      );
    }
    if (error) {
  ...
  return (
    <div className={styles.main}>
      ...
      <h1 className={styles.heading + " !text-2xl font-bold text-left "}>
        Response Sheet Submissions
      </h1>
