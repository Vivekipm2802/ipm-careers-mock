...
import { Button, Chip, Divider, Dropdown, DropdownItem, DropdownMenu, DropdownTrigger, Image, Input, Modal, ModalBody, ModalContent, ModalFooter, ModalHeader, Popover, PopoverContent, PopoverTrigger, Select, SelectItem, Skeleton, Slider, Spacer, Spinner, Switch, Tooltip } from "@nextui-org/react";
import ImageUploader from "./ImageUploader";
import dynamic from "next/dynamic";
import { useRouter } from "next/router";
import { toast } from "react-hot-toast";
import { AlertOctagon, ArrowRight, BookAIcon, ChartBarIncreasing, ChartSplineIcon, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, Edit2, Eye, GripVertical, Info, LockIcon, PencilIcon, RefreshCw, Space, Star, Trash2, Video, X } from "lucide-react";
...
async function getLeaderBoard(a){
  ...
  const {data,error} = await supabase.from('plays').select('*').eq('test_uuid',a).order('score',{ascending:false}).limit(10)
  if(data){
...
handler(data)
  ...
  async function getPlays(a){
    const {data,error} = await supabase.from('plays').select('*,test_uuid(*)').in('test_uuid',a.map((i)=>{return i.uuid})).match({'user':userDetails?.email,isPassed:true})
    if(data){
      console.log(data)
      setPlays(data)
    }
  ...
  async function addNewLevel(a,b){
    const requiredFields = ["title",  "difficulty",  "time", "image", "description", "objective"];
...
toast.error('Duplicate Name Level Already exists - please Change');
  ...
  async function DeleteLevelQuestionbyId(a,b){
    ...
    if(!error){
      getData('questions','title,id','parent',b,(e)=>{setQuestions(e)},({errortext})=>{console.log(errortext)})
...
async function updateGameCategory(a,b){
    ...
    if(a == undefined){
      ...
      const {error} = await supabase.from('m_categories').update({
        title:a.gct,
        icon:a.gci
      }).eq('id',b);
      if(!error){
  ...
  async function getLevels(a){
    ...
    const {data,error} = await supabase.from('levels').select('*,questions!questions_parent_fkey(id)').eq('parent',a).order('created_at',{ascending:true})
  ...
  async function getGameCategories(){
    const {data,error} = await supabase.from('m_categories').select('*').order('created_at',{ascending:true})
  ...
  async function getCategories() {
    const { data, error } = await supabase.from("categories").select("*").eq('parent',group);
...
async function getOrderQuestions(a,handler,handler2){
  ...
  const {data,error} = await supabase.from('questions').select('title,id,seq,question').eq('parent',a).order('seq',{ascending:true});
  if(data){
    setQuestions(data)
    handler()
  }
...
async function updateOrder(a,b,handler){
  ...
  return (
    <div className="h-full relative flex flex-col overflow-hidden">
      <Modal isOpen={activeResult} onClose={()=>{setActiveResult(undefined)}}>
        <ModalContent>
          <ModalHeader>Your Attempts</ModalHeader>
          <ModalBody>
            {plays && plays.filter(item=>item.test_uuid?.id == activeResult)?.map((i,d)=>{
              return <div className="flex flex-row items-center justify-between">
                <div className="flex-1">
                  <h2 className="text-xs"><strong className="text-primary">{CtoLocal(i?.created_at)?.time} {CtoLocal(i?.created_at)?.amPm}</strong><br/>{CtoLocal(i?.created_at)?.date} {CtoLocal(i?.created_at)?.monthName} {CtoLocal(i?.created_at)?.year}</h2>
                </div>
                <div className="flex flex-row items-center justify-end">
                <Button endContent={<ChartBarIncreasing size={16}></ChartBarIncreasing>} as={Link} target="_blank" href={`/test/analytics/${i?.uid}`} size="sm" color="success" >View Result & Analytics</Button>
               </div>
              </div>
            })}
          </ModalBody>
          <ModalFooter>
            <Button size="sm" color="danger" variant="flat" onPress={()=>{setActiveResult(undefined)}}>Close</Button>
            
          </ModalFooter>
        </ModalContent>
      </Modal>
<Modal placement="center" isOpen={video != undefined} onClose={()=>{setVideo()}}>
  <ModalContent className=" overflow-hidden w-full max-w-[600px]">
    <ModalHeader>{activeLevel?.title}</ModalHeader>
    <ModalBody className="p-4">
      <iframe className="w-full aspect-video rounded-xl" src={video}></iframe>
    </ModalBody>
  </ModalContent>
</Modal>
<Modal onOpenChange={(e)=>{e== true ? '':setQuestions()}} isDismissable={false} isOpen={reorderModal} onClose={(e)=>{setReorderModal(false)}}>

  <ModalContent className="font-sans">
    <ModalHeader>
      Re Arrange Questions (Drag n Drop)
    </ModalHeader>
    <ModalBody>
<Reorder.Group axis="y"  values={questions??[]}  onReorder={(e)=>{setQuestions(e),updateOrder(e.map((item,index)=>({id:item.id,seq:index})),"questions",(item)=>{getOrderQuestions(item[0]?.parent,()=>{},()=>{}) })}} className="max-h-[60vh] overflow-y-auto overflow-x-hidden pr-2">
  {questions && questions.map((i,d)=>{
    return <DraggableItem key={i.id} value={i} gripItem={(c)=>(<GripVertical onPointerDown={(e) => c.start(e)} color="#aaa" className=" cursor-grab reorder-handle"></GripVertical>)}
    ...
    >
     <div className="flex ml-2 text-xs flex-row items-center justify-start">
      ...
      <Tooltip content={<div dangerouslySetInnerHTML={{__html:i?.question}} className="text-xs [&_span]:!text-xs max-h-[70px] overflow-y-auto"></div>}>
      <Info className="ml-auto"></Info></Tooltip>
    </DraggableItem>
  })}
</Reorder.Group>
    </ModalBody>
    <ModalFooter>
      <Button color="danger" onPress={()=>{setReorderModal(false)}}>Close </Button>
    </ModalFooter>
  </ModalContent>
</Modal>

<Modal scrollBehavior={"outside"} isDismissable={false} size='3xl' className="flex mdl flex-col gap-1 text-center items-center" onClose={()=>{setQuestionToggle(false),setQuestions()}} placement="bottom-center"  isOpen={questionToggle} >
...
<Button className='mr-auto' color='primary' onPress={()=>{setEditMode(false),setEditQuestionData()}}>Back to Questions</Button>
...
{questions != undefined && questions.map((i,d)=>{
  return <div className='w-full text-left flex flex-row align-middle justify-start items-center'><h2 className='font-bold'>{d+1}){i.title}</h2> <p onClick={()=>{setEditMode(true),getData('questions','*','id',i.id,(e)=>{setEditQuestionData(e[0])})}} className="rounded-full bg-blue-300 font-medium text-xs mx-1 ml-3 cursor-pointer px-2 py-0.5">Edit Question</p> <p onClick={()=>{DeleteLevelQuestionbyId(i.id,parentMain)}} className='bg-red-500 cursor-pointer w-auto rounded-xl text-white text-xs px-3 p-1 inline-block mx-2'>Delete</p></div>
})}</>}
  </ModalBody>
  ...
  {editMode == true && editQuestionData != undefined ? <div key={editQuestionData.id}>
    ...
    <Button isDisabled={editQuestionData?.video != undefined && editQuestionData?.video?.length > 2 ? false : true} className='w-auto mr-auto' color='primary'><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
></iframe>
  :''}
   ...
   <ImageUploader data={{image:i?.image}}  size="small" onUploadComplete={(e)=>{updateOrAddOption2(d,e,'image')}}></ImageUploader></div>
    ...
    <ImageUploader size="small" data={{image:i?.popupimage}} onUploadComplete={(e)=>{updateOrAddOption2(d,e,'popupimage')}}></ImageUploader>
    <h2>Popup Text</h2>
    <CustomEditor 
  ...
  options: { ...res.options, losetext: e }
    ...
    <h2>Explanation</h2>
    <CustomEditor data={editQuestionData?.explanation || "<strong>Write your Explanation Here...</strong>"} value={editQuestionData?.explanation || ""} onChange={(e)=>{setEditQuestionData(res=>({...res,explanation:e}))}}></CustomEditor>
    ...
    <Input label="Explanation Video" placeholder='Enter Video URL' value={editQuestionData?.explanationvideo || ""} onChange={(e)=>{setEditQuestionData(res=>({...res,explanationvideo:e.target.value}))}}></Input>
   ...
   <div className='h-auto'> {editlatex == true ? 
     <EditableMathField mathquillDidMount={mathField => {
        ...
        <Button color='primary' onClick={()=>{setEditLatext(!editlatex)}}>{editlatex ? 'Save Edit' : 'Edit Now'}</Button>
        </div>
    <h2>Hint</h2>
    ...
    <Button onClick={()=>{
      updateData('questions',{
        ...
        explanationvideo:editQuestionData?.explanationvideo || "",
        ...
        getData('questions','title,id','parent',parentMain,(e)=>{setQuestions(e),setQuestionToggle(false)},({errortext})=>{console.log(errortext)})
      },()=>{})
    }} className='text-left w-auto mr-auto mt-2' color='primary'>
 Update Question</Button>
    ...
    <h2>Question Content</h2>
    <CustomEditor label="Question Title" placeholder='Enter Question Title' data={addNewQuestion?.question || "<p>Enter Question</p>"} value={addNewQuestion?.question || ""} onChange={(e)=>{setAddNewQuestion(res=>({...res,question:e}))}}></CustomEditor>
    ...
    <Popover><PopoverTrigger>
    <Button isDisabled={addNewQuestion?.video != undefined && addNewQuestion?.video?.length > 2 ? false : true} className='w-auto mr-auto' color='primary'><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M9.5 9.38455V14.6162C9.5 15.1858 10.1099 15.5475 10.6097 15.2743L15.3959 12.6582C15.9163 12.3737 15.9162 11.6263 15.3958 11.3419L10.6097 8.72641C10.1099 8.45328 9.5 8.81499 9.5 9.38455ZM5.25 3C3.45507 3 2 4.45507 2 6.25V17.75C2 19.5449 3.45507 21 5.25 21H18.75C20.5449 21 22 19.5449 22 17.75V6.25C22 4.45507 20.5449 3 18.75 3H5.25ZM3.5 6.25C3.5 5.2835 4.2835 4.5 5.25 4.5H18.75C19.7165 4.5 20.5 5.2835 20.5 6.25V17.75C20.5 18.7165 19.7165 19.5 18.75 19.5H5.25C4.2835 19.5 3.5 18.7165 3.5 17.75V6.25Z" fill="currentColor"/>
</svg>
Preview Video</Button></PopoverTrigger>
<PopoverContent>
...
></iframe>
  :''}
   ...
   <ImageUploader size="small" onUploadComplete={(e)=>{updateOrAddOption(d,e,'image')}}></ImageUploader></div>
    ...
    <h2>Popup Image (Win/Lose Image)</h2>
    <ImageUploader size="small" onUploadComplete={(e)=>{updateOrAddOption(d,e,'popupimage')}}></ImageUploader>
    <h2>Popup Text</h2>
    <CustomEditor 
  ...
  options: { ...res.options, losetext: e }
    ...
    <h2>Explanation Video</h2>
    <Input label="Explanation Video" placeholder='Enter YT Embed or Video URL' value={addNewQuestion?.explanationvideo || ""} onChange={(e)=>{setAddNewQuestion(res=>({...res,explanationvideo:e.target.value}))}}>
    ...
    <Button onClick={()=>{
      insertData('questions',{
        ...
        explanationvideo:addNewQuestion?.explanationvideo || "",
    ...
    }} className='text-left text-sm w-auto mr-auto' color='primary'><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M11.7498 3C12.1295 3 12.4434 3.28201 12.4931 3.64808L12.5 3.74985L12.5012 11H19.7543C20.1685 11 20.5043 11.3358 20.5043 11.75C20.5043 12.1297 20.2221 12.4435 19.8561 12.4932L19.7543 12.5H12.5012L12.5032 19.7491C12.5033 20.1633 12.1676 20.4993 11.7534 20.4993C11.3737 20.4993 11.0598 20.2173 11.0101 19.8512L11.0032 19.7494L11.0012 12.5H3.7522C3.33798 12.5 3.0022 12.1642 3.0022 11.75C3.0022 11.3703 3.28435 11.0565 3.65043 11.0068L3.7522 11H11.0012L11 3.75015C10.9999 3.33594 11.3356 3 11.7498 3Z" fill="currentColor"/>
 ...
 Add Question</Button>
     ...
     <motion.div className="flex flex-col overflow-hidden w-full h-full items-stretch justify-stretch"
     key={view}
     ...
     >
      ...
      /* View 1 */
        ...
        >
          <div className={"w-full absolute lg:sticky lg:top-0 lg:w-[400px] z-[1] transition-all lg:transition-none overflow-y-auto -translate-x-full lg:translate-x-0 lg:transform-none h-full bg-gray-50 overflow-hidden flex flex-col items-start justify-start "  + (drawerActive ? ' !translate-x-0':'')}>
          <div className="flex mb-0 flex-col w-full text-left justify-start align-top items-start">
                  <div className="flex  bg-gradient-to-r from-slate-50 sticky top-0 z-[99] to-slate-200 text-black font-bold p-4 flex-row items-center justify-between w-full">
                   <Button size="sm" onPress={()=>{onBack()}} color="primary" variant="bordered" startContent={<ChevronLeft></ChevronLeft>}> Back </Button>
                    </div></div>
          {categories &&
           categories.map((i, d) => {
              return (
                <div className="flex mb-0 flex-col w-full text-left justify-start align-top items-start">
                  <div className="flex  bg-gradient-to-r from-secondary-500 sticky top-0 z-[99] to-yellow-200 text-black font-bold p-4 flex-row items-center justify-between w-full">
                   ...
                   <Star size={16}></Star> <Spacer x={2}></Spacer> <h2 className="font-medium mr-5">{i.title}</h2></div>
                    {role == "admin" ? (
                              ...
                              <Button size="sm"
                                ...
                                onClick={() => {
                                  categoryTitle != undefined
                                    ? updateCategoryTitle(i.id, categoryTitle)
                                    : "";
                                }}
                              >
                                Update
...
<Spacer x={2}></Spacer>
                        <Button size="sm" color="danger" isIconOnly onPress={()=>{ deleteParentCategory(i.id)}}>
<Trash2 color="white" size={16}></Trash2>
                  ...
                  <ul className="flex flex-col w-full">
                    ...
                    {gamecategories &&
                      categories &&
                      gamecategories.filter(item=>item.parent == i.id).map((z, v) => {
                        return (
                          /* Sub Category Item */<>
                          <div  onClick={() => {
                              z.id == selectedLevel ? (setSelectedLevel()):(setSelectedLevel(z.id),getLevels(z.id))
                            }} className={"relative bg-white  p-4 py-1 w-full mb-0 border-b-1 cursor-pointer  transition-all flex flex-row text-center " + (selectedLevel == z.id  ? ' !bg-secondary-50':'')}>
                           ...
                           <p
                             ...
                             <ChevronDown className={"transition-all " +(selectedLevel == z.id  ? 'rotate-180':'')}></ChevronDown>
                             <Spacer x={2}></Spacer>
                              {role == "admin" ? (
                                <Popover
                                  onOpenChange={(e) => {
                                ...
                                >
                                  <PopoverTrigger>
                                    <Button isIconOnly size="sm" color="success">
                                      <Edit2 size={16} color="white"></Edit2>
                                    </Button>
                                  </PopoverTrigger>
                                  <PopoverContent>
                                    <Input
                                    ...
                                    <Button
                                      ...
                                      color="primary"
                                      onPress={() => {
                                        updateGameCategory(editable, z.id);
                                      }}
                                    >
                                      Update
                                    </Button>
                          ...
                          <div className={"relative flex flex-col overflow-y-auto w-full h-0 overflow-hidden my-0  max-h-60 md:max-h-full " + (selectedLevel == z.id  ? ' py-4 !h-full':'')}>
               {levelLoading && levelData == undefined ? <Skeleton  className="w-full p-8 rounded-xl"></Skeleton>:''}
               {(levelData == undefined || levelData?.length == 0) && !levelLoading && <div className="w-full text-center p-2"><p className="w-full p-2 flex flex-row items-center justify-center py-2 rounded-xl shadow-md bg-gray-50 border-dashed border-1">
                <AlertOctagon size={16}></AlertOctagon>
                <Spacer x={2}></Spacer>
                No Test Found in this category</p></div>}
               {levelData &&
                  levelData.map((i, d) => {
                    return (
                     <div key={i.id} className="px-2 mb-2 flex-1"> <div
                        onClick={() => {
                          setActiveLevel(i),setDrawerActive(false),getLeaderBoard(i.uuid)
                        }}
                        className={`${
                          d == 0 ||
                          role == "admin" ||
                          (plays &&
                            plays.some(
                              (item) => item.isPassed && levelData[d-1]?.id == item?.test_uuid?.id
                            ))
                            ? ""
                            : "pointer-events-none opacity-70 grayscale cursor-not-allowed" 
                        } w-full flex flex-col justify-between cursor-pointer h-auto text-left shadow-sm p-3 rounded-md hover:scale-[0.995]  bg-white  transition-all hover:shadow-md ` + (activeLevel?.id == i.id ? ' border-1 border-secondary bg-secondary-50':'')}
                      >
                        
                        <div className="flex flex-row justify-between w-full">
                          <h2 className="font-bold flex flex-row items-center  justify-start text-md">{i.title}</h2>

                          {activeLevel?.id == i.id && role != "admin" && <CheckCircle2 className="fill-lime-500" stroke="white"></CheckCircle2> }
                          {
                          d == 0 ||
                          role == "admin" ||
                          (plays &&
                            ...
                            ? ""
                            : <div className="flex flex-col items-center justify-center"><LockIcon size={16}></LockIcon></div>
                        }
                          {role == "admin" ? (
                              ...
                              <PopoverContent className="max-w-[500px] max-h-[500px] sf overflow-y-auto flex flex-col justify-start items-start align-middle">
                                ...
                                <Input
                                  ...
                                  onChange={(e) => {
                                    setEditable((res) => ({
                                      ...res,
                                      lv: convertToYouTubeEmbed(e.target.value),
                                    }));
                                  }}
                                ></Input>
                                <Popover>
                                ...
                                <Switch
                                  ...
                                  isSelected={editable?.calculator_allowed ?? false}
                                  onValueChange={(e) => {
                                    setEditable((res) => ({
                                      ...res,
                                      calculator_allowed:  e,
                                    }));
                                  }}
                                >Is Calculator Allowed ? </Switch>
                                <Switch
                                  className="my-2"
                                 
                                  
                                
                                  isSelected={editable?.is_scientific ?? false}
                                  onValueChange={(e) => {
                                    setEditable((res) => ({
                                      ...res,
                                      is_scientific: e,
                                ...
                                >Should be Scientific Calculator ? </Switch>
                                <Input
                                  className="my-2"
                                  autoFocus
                                  ...
                                  value={editable?.ltime ?? ''}
                                  onChange={(e) => {
                                    setEditable((res) => ({
                                      ...res,
                                      ltime: e.target.value,
                                    }));
                                  }}
                                ></Input>
                                <Slider minValue={5} onChange={(e)=>{setEditable(res=>({...res,ltime:e}))}} value={editable?.ltime} maxValue={180}></Slider>

                                <ImageUploader
                                  size="small"
                                  data={{ image: editable?.li || "" }}
                                  onUploadComplete={(e) => {
                                    setEditable((res) => ({ ...res, li: e }));
                                  }}
                                ></ImageUploader>
                                ...
                                <Button
                                  ...
                                  color="primary"
                                  onPress={() => {
                                    updateLevel(editable, i.id, i.parent);
                                  }}
                                >
                                  Update Level
                                </Button>
                            ...
                            <Tooltip content="Re-order Questions" size="sm">
                            <Button size="sm" onPress={()=>{
                            getOrderQuestions(i.id,()=>{setReorderModal(true)},()=>{})
                            }} isIconOnly className="ml-2 shadow-md border-1 from-gray-50 to-white bg-gradient-to-r">
                              <RefreshCw size={16}></RefreshCw>
                            </Button></Tooltip>
                            <Tooltip content="Add Questions" size="sm">
                              <Button
                                onPress={() => {
                                  setParentMain(i.id),
                                    getData(
                              ...
                              size="sm"
                                ...
                                onPress={() => {
                                  DeleteLevelbyId(i.id, i.parent);
                                }}
                                isIconOnly={true}
                                className="text-white border-gray-50 from-danger to-secondary bg-gradient-to-b border-1 mx-0  shadow-sm shadow-danger"
                              >
                                <svg width="18" height="18" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M21.5 6a1 1 0 0 1-.883.993L20.5 7h-.845l-1.231 12.52A2.75 2.75 0 0 1 15.687 22H8.313a2.75 2.75 0 0 1-2.737-2.48L4.345 7H3.5a1 1 0 0 1 0-2h5a3.5 3.5 0 1 1 7 0h5a1 1 0 0 1 1 1Zm-7.25 3.25a.75.75 0 0 0-.743.648L13.5 10v7l.007.102a.75.75 0 0 0 1.486 0L15 17v-7l-.007-.102a.75.75 0 0 0-.743-.648Zm-4.5 0a.75.75 0 0 0-.743.648L9 10v7l.007.102a.75.75 0 0 0 1.486 0L10.5 17v-7l-.007-.102a.75.75 0 0 0-.743-.648ZM12 3.5A1.5 1.5 0 0 0 10.5 5h3A1.5 1.5 0 0 0 12 3.5Z" fill="#fff"/></svg>
                              </Button>
...
<Spacer x={2}></Spacer>
                  ...
                  <Popover className="flex flex-col items-start align-middle justify-start overflow-hidden">
                    <PopoverTrigger autoFocus={false}>
                      <div className="w-full my-2 flex flex-row items-center justify-center align-middle p-2 border-1 border-gray-300 rounded-xl px-5 shadow-sm bg-white cursor-pointer hover:bg-gray-200 transition-all mx-auto">
                        ...
                        <p className="text-xs">Add New Level to this Category</p>
                        <Spacer y={5}></Spacer>
                      </div>
                    </PopoverTrigger>
                     ...
                     {/*  <Dropdown>
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          setNewLevelData((res) => ({
                            ...res,
                            video: convertToYouTubeEmbed(e.target.value),
                          }));
                        }}
                      ></Input>
                      <Popover>
                        <PopoverTrigger>
                          <Button
                            className="my-2"
                            isDisabled={newLevelData?.video == undefined}
                            color="primary"
                          >
                            <svg
                              width="24"
                      ...
                      <ImageUploader
                        size="small"
                        data={{ image: newLevelData?.image || "" }}
                        onUploadComplete={(e) => {
                          setNewLevelData((res) => ({ ...res, image: e }));
                        }}
                      ></ImageUploader>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          addNewLevel(newLevelData, selectedLevel);
                        }}
                      >
                        Add Level
                        {loading == true ? (
          ...
          {role == "admin" ? (
            <Dropdown className="sf">
              <DropdownTrigger>
                <Button color="primary" size="sm" className="w-full mt-auto z-[999] flex-shrink-0 sticky bottom-0 rounded-0">Add New Category</Button>
              </DropdownTrigger>
...
<div className={"flex-1 absolute z-0 left-0 top-0 w-full lg:w-auto h-full lg:relative bg-gray-100 p-0 lg:p-4 flex flex-col items-start justify-start rounded-xl overflow-y-auto overflow-x-hidden lg:!transform-none translate-x-0 "}>
{!activeLevel && <div className="w-full h-full flex flex-col items-center justify-center text-gray-600"><PencilIcon size={48}></PencilIcon>
<Spacer y={2}></Spacer>
       <h2 className="text-md max-w-[20ch]">Please Select a test from list to view</h2>
       </div> }
<AnimatePresence mode="wait">
        ...
        {activeLevel  ?<>
          ...
          <Button onPress={()=>{setDrawerActive(true),setActiveLevel()}} size="md" startContent={<ArrowRight size={16} className=" -rotate-180"></ArrowRight>} className="bg-white mb-4 shadow-md flex lg:hidden border-1 rounded-full text-black ">Back to Modules
       
       </Button>
       {activeLevel?.image &&   <div className="w-full relative aspect-video overflow-hidden rounded-xl"><Image fetchPriority="high" classNames={{wrapper:"w-full h-full !max-w-[unset]"}} className="w-full z-0 relative h-full object-cover" src={activeLevel?.image}/>
...
<div className=" absolute left-0 bottom-0 w-full h-[70%] bg-gradient-to-t z-0 from-[#000a] to-[50%] to-transparent"></div>
       ...
       {activeLevel?.video && <Button onPress={()=>{setVideo(activeLevel?.video)}} size="lg" endContent={<Video></Video>} className="bg-gradient-to-r from-purple-800 to-secondary text-white shadow-md border-1 relative rounded-full  mr-3 ">Watch Video
       
       </Button>}
        <Button as={Link} target="_blank" href={`/test/${activeLevel?.uuid}`} size="lg" startContent={ <div className="w-3 h-3 rounded-full  animate-ping bg-secondary -z-[1]"
        ></div>} endContent={<ArrowRight></ArrowRight>} className="bg-white shadow-md border-1 relative rounded-full text-black ">Start Test
       
        </Button>

        {plays && plays?.filter(item=>item.test_uuid.id == activeLevel?.id)?.length > 0 && <div className="ml-4 flex  flex-row items-center justify-start"><Button onPress={()=>{setActiveResult(activeLevel?.id)}} size="lg" startContent={<Eye size={24}></Eye>} className="bg-warning shadow-md border-1 relative rounded-full text-black ">Results & Analytics
         ...
         <h2 className=" text-3xl font-bold text-primary my-4">{activeLevel?.title}</h2>
         <div className="flex flex-row flex-wrap items-center justify-start">
         <Chip className="mr-2 mb-2" variant="flat" size="md" color="primary">Difficulty : {difficulty.find(item=>item.value == activeLevel?.difficulty)?.title}</Chip>
         <Chip className="mr-2 mb-2" variant="flat" size="md" color="success">Question Count : {activeLevel?.questions?.length}</Chip>
         <Chip className="mr-2 mb-2" variant="flat" size="md" color="primary">Test Time : {convertSecondsToMinutes(activeLevel?.time)}</Chip>
         </div>
         ...
         <div className="w-full rounded-xl text-left bg-white shadow-sm p-4">
          <h2 className="text-2xl font-semibold text-primary">Level Description</h2>
          <div dangerouslySetInnerHTML={{__html:activeLevel?.description}}></div>
         </div>
         <Spacer y={2}></Spacer>
       
         <div className="w-full rounded-xl text-left bg-white shadow-sm p-4">
          <h2 className="text-2xl font-semibold text-primary">Level Objective</h2>
          <div dangerouslySetInnerHTML={{__html:activeLevel?.objective}}></div>
          
         </div>
         </div>
         <Leaderboard scores={leaderboardData ?? []}></Leaderboard>
         </div>
         </>
         :''}
        </motion.div>
