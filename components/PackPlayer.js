...
function ChapterAccordion({
  ...
  // Subs that have no video are still shown as a row (no-op click).
      ...
      if (vids.length === 0) {
        ...
        vids.forEach((v, idx) =>
          out.push({
            kind: "video",
            id: v.id,
            label: vids.length === 1 ? s.title : `${s.title} · ${idx + 1}`,
            duration: v.duration,
            video: v,
          }),
        );
...
// Supports YouTube, Vimeo, generic mp4. Falls back to raw URL.
function renderVideoEmbed(video) {
  const url = video.url || video.video_url || video.vurl || "";
  if (!url) {
  ...
  // YouTube
  const ytMatch =
    lower.match(/youtube\.com\/watch\?v=([^&]+)/) ||
    lower.match(/youtu\.be\/([^?&]+)/) ||
    lower.match(/youtube\.com\/embed\/([^?&]+)/);
  if (ytMatch) {
    const id = ytMatch[1];
    return (
      <iframe
