...
import { Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Button, Input, Textarea, Radio, 
    Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Spacer, DatePicker, DateInput, Switch
...
export default function RCManager() {
  ...
  async function handleSaveQuestion() {
    try {
      ...
      console.error(`Error ${modalMode === 'create' ? 'creating' : 'updating'} question:`, error)
    }
  }
  ...
  return (
    <div className="container w-full h-full text-left mx-auto px-4 py-8 overflow-y-auto">
     <div className='flex flex-row items-center justify-between'>
      ...
      <Button 
      size='sm'
      startContent={<Sparkle size={16} fill="black" className="rotate-45 "></Sparkle>}
        color="secondary" 
        onPress={() => {
          generateRC()
        }}
          ...
          {questions && questions.map((question) => (
            <TableRow key={question?.id}>
                ...
                <div className="flex bg-green-50 rounded-xl p-2 flex-col items-center justify-center text-center">
                        <h2 className="text-xl font-bold text-green-500">{CtoLocal(question?.date).date}</h2>
                        <p className="text-green-700 text-xs">{CtoLocal(question?.date).monthName}</p>
                    </div>
                </TableCell>
              ...
              <TableCell>
                ...
                <Spacer x={2}></Spacer>
                <Button size="sm" color="danger" onPress={() => handleDeleteQuestion(question.id)}>Delete</Button></div>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
...
function QuestionModal({ isOpen, onClose, question, setQuestion, onSave, mode }) {
    return (
      <Modal scrollBehavior="inside" isOpen={isOpen} onClose={onClose}>
        <ModalContent>
          <ModalHeader>{mode === 'create' ? 'Create New Question' : 'Edit Question'}</ModalHeader>
          <ModalBody>
            <Input
              label="Title"
  ...
  onChange={(e) =>
    setQuestion((prev) => ({ ...prev, content: e }))
  }
  className="mb-2"
/>
            ...
            {(question?.type === 'options' || mode == 'create') && (
              <div className="mb-2">
                ...
                {question?.options?.map((option, index) => (
                 ...
                 <Switch size='sm' color='success' onValueChange={(value)=>{setQuestion(res=>({...res,answer_key:{...res.answer_key,correct:value == true ? index:res.answer_key.correct}}))}} isSelected={(index == question?.answer_key?.correct)} title='is Correct?'></Switch>
                    <Input
                      value={option}
                      onChange={(e) => {
                ...
                <Button
                  size="sm"
                  color="primary"
                  onPress={() => setQuestion({...question,options: [...(Array.isArray(question.options) ? question.options : []), '']})}
                  className="mt-2"
                >
                  Add Option
                </Button>
  ...
  <Input
    onChange={(e) => {
      ...
      setQuestion((res) => ({ ...res, date: selectedDate.toISOString() })); // Save as UNIX timestamp in seconds
    }}
    type="date"
    label="Date"
...
/>
          ...
          <ModalFooter>
            <Button color="primary" onPress={onSave}>{mode === 'create' ? 'Create' : 'Save'}</Button>
