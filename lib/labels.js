...
function shortSectionName(title) {
  ...
  s = s
    ...
    .trim();
  s = s || raw;
  // 2026-09 (owner feedback on analytics v5): canonicalise the long
  // subject names to the exam's own shorthand so sparkline rows and
  // strip cells stay compact ("Verbal Ability" → "VA").
  var canon = {
  ...
  var mapped = canon[s.toLowerCase()];
  return mapped || s;
}
