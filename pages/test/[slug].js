...
const Game = () => {
  ...
  const {data,error} = await serversupabase.from('plays').select('id').eq('test_uuid',router.query.slug).eq('isPassed',true).eq('user',uid);
...
async function getQuestions(){
    ...
    const {data,error} = await supabase.from('levels').select('*,questions!questions_parent_fkey(*)').eq('uuid',router.query.slug).order('seq',{foreignTable:"questions",ascending:true})
if(data){
...
async function getLeaderboard(a){
  const {data,error} = await supabase.from('plays').select('id,created_at,score,user,name,isPassed,test_uuid').eq('test_uuid',a).order('score',{ascending:false}).limit(20)
  if(data && data.length != 0){   
      setLeaderBoard(data);
  }
}
...
return <div className='w-full h-screen flex flex-col items-center justify-center'>
  ...
  <Button size="sm" color='secondary' onPress={()=>{router.push('/')}}>Go Back</Button>
</div>
}
  return (
  ...
  <ModalContent>
    ...
    <ModalBody>
      You have answered {report?.length ?? 0} questions out of total {questions?.length} questions
    </ModalBody>
    <ModalFooter className='flex flex-row justify-start'>
    <Button color='danger' size='sm' onPress={()=>{setConfirmModal(false)}}>Cancel</Button>
      <Button size='sm' color='default' className='from-primary border-1 border-white shadow-md shadow-primary-400 to-primary-600 bg-gradient-to-r text-white'  onPress={()=>{handleComplete()}}>Confirm</Button>
          ...
          <>
              ...
              <div className="w-full flex flex-col justify-center align-middle items-stretch h-full relative">
                ...
                report={report}
                  ...
                  isMarked={report?.some(item=>item?.id == questions[level]?.id && (item?.status == 'review'))}
                />
              </div>
...
{report &&
            report.filter((item) => item.isCorrect == true).length >
              ...
              <p className="font-bold text-red-600 text-center text-sm">
                ...
                order to pass the test {parentData?.parent?.title}
              </p>
            )}
           {submitted &&  <Button as={Link} href={`/test/analytics/${submitted?.uid}`} size='lg' className=' from-primary-500 to-primary-700 bg-gradient-to-r mx-auto text-white'>View Analysis</Button>}
</div>
<div className='w-full bg-gray-100 flex flex-col items-start justify-start flex-1 h-full overflow-y-auto p-0'>
         ...
         {gamestate < 2 && <QuestionBrowser switchQuestion={(e)=>{setLevel(questions?.findIndex(item=>item.id == e))}}  gamestate={gamestate} questions={questions} report={report} sideBarActive={sideBarActive} setSidebarActive={(e)=>{setSidebarActive(e)}}></QuestionBrowser>}
      </div>
