...
import { Button, Spacer, CircularProgress, Spinner, Modal, ModalFooter, ModalHeader, ModalContent, ModalBody, Input } from '@nextui-org/react';
...
if(await getRole(formData?.email) != "user"){
    toast.error("this account doesn't belong to a user")
    return null
}
...
async function forgotPassword(a){
  ...
  const { data, error } = await supabase.auth.resetPasswordForEmail(a,{redirectTo:'https://study.ipmcareer.in/login'})
...
<Button color='primary' onPress={()=>{updatePassword(fpUpdate)}}>Update Password</Button>
    ...
    <Modal placement='center' className='sf overflow-hidden' isOpen={fpModal} backdrop='opaque' onClose={()=>{setFPModal(false)}} isDismissable={false}  classNames={{backdrop:"opacity-10 bg-overlay/5"}} scrollBehavior="inside">
<ModalContent>
{(onClose)=>(<>
    <ModalHeader className={`flex flex-col gap-1 justify-start items-start text-black`}>
...
<Button  className='text-white bg-gradient-purple'  onPress={()=>{forgotPassword(fpData)}}>Send Reset Link</Button>
...
<div className='w-full max-w-[1000px] z-10 relative  shadow-lg border-1 bg-white border-white  h-full max-h-[80vh]  md:max-h-[95vh] overflow-hidden my-auto rounded-xl mx-auto flex flex-row justify-between'>
        ...
        <img className={"relative w-[50vw] md:w-[20vw] mr-auto"} width={100} src='/newlog.svg'/>
       {/*  <div></div> */}
            
            <div className={styles.form + " " + (isChanging? styles.hid : '')}>
       <h2 className='!text-md ' style={{color:'var(--brand-col1)'}}>{isSignUp? "Create a new account to start learning on our panel":"Login to your account !"}</h2>
      
       <Spacer y={1}></Spacer>
    {isSignUp ?  <input name={"name"} autoComplete='name' className={styles.input} placeholder={"Enter your Full Name"} type={"text"} value={formData && formData.fullname} onChange={(e)=>{setFormData(res=>({...res,fullname:e.target.value})) }}/>:''}
     <input name={"email"} autoComplete='email' className={styles.input} placeholder={"Enter your Email Address"} type={"text"} value={formData && formData.email} onChange={(e)=>{setFormData(res=>({...res,email:e.target.value})) }}/>
     {isSignUp ?<input name={"phone"} autoComplete='phone' className={styles.input} placeholder={"Enter your Phone Number"} type={"text"} maxLength={10} value={formData && formData.phone} onChange={(e)=>{setFormData(res=>({...res,phone:e.target.value})) }}/>:''}
     {isSignUp ?<input name={"city"} autoComplete='city' className={styles.input} placeholder={"Enter your City"} type={"text"} value={formData && formData.city} onChange={(e)=>{setFormData(res=>({...res,city:e.target.value})) }}/>:''}
    ...
    <div className={styles.password}><div className={styles.tog} onClick={()=>{isPasswordVisible?setIsPasswordVisible(false):setIsPasswordVisible(true)}}>{!isPasswordVisible?<svg width="24" height="24" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 9.005a4 4 0 1 1 0 8 4 4 0 0 1 0-8ZM12 5.5c4.613 0 8.596 3.15 9.701 7.564a.75.75 0 1 1-1.455.365 8.503 8.503 0 0 0-16.493.004.75.75 0 0 1-1.455-.363A10.003 10.003 0 0 1 12 5.5Z" fill="#BDC3C8"/></svg>:<svg width="24" height="24" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M2.22 2.22a.75.75 0 0 0-.073.976l.073.084 4.034 4.035a9.986 9.986 0 0 0-3.955 5.75.75.75 0 0 0 1.455.364 8.49 8.49 0 0 1 3.58-5.034l1.81 1.81A4 4 0 0 0 14.8 15.86l5.919 5.92a.75.75 0 0 0 1.133-.977l-.073-.084-6.113-6.114.001-.002-6.95-6.946.002-.002-1.133-1.13L3.28 2.22a.75.75 0 0 0-1.06 0ZM12 5.5c-1 0-1.97.148-2.889.425l1.237 1.236a8.503 8.503 0 0 1 9.899 6.272.75.75 0 0 0 1.455-.363A10.003 10.003 0 0 0 12 5.5Zm.195 3.51 3.801 3.8a4.003 4.003 0 0 0-3.801-3.8Z" fill="#BDC3C8"/></svg>}</div> <input name={"password"} autoComplete='password' className={styles.input} placeholder={"Enter Password"} type={isPasswordVisible ? "text" : "password"}  value={formData && formData.password} onChange={(e)=>{setFormData(res=>({...res,password:e.target.value})) }}/></div>
<Spacer y={1}></Spacer>
<Button className='text-white gradient-purple bg-gradient-purple' auto onClick={()=>{isSignUp?handleSignUp():handleSignIn()}} >{isSignUp? "Sign Up":"Sign In"}{loading? <><Spacer x={0.5}></Spacer><Spinner size="sm"  color={"default"} ></Spinner></>:''}</Button>
<p className='text-blue-500 text-sm my-2 cursor-pointer hover:text-blue-700' onClick={()=>{setFPModal(true)}}>Forgot Password?</p>


<p className={styles.txt + " text-xs my-2 bg-white rounded-lg shadow-sm p-2 py-4"}>{isSignUp? "Already have an Account ? ":"Want to create a new account ? "}<span className={styles.log + " !text-secondary border-1 border-secondary p-1 rounded-md hover:bg-secondary transition-all hover:!text-white"} onClick={(e)=>{Switch() }}>{isSignUp? "Sign In":"Sign Up Now"}</span></p>
