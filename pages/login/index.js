// ============================================================
// Student Login — Phase 2 (final)
// Centered single-card design, matches login-preview-v2.html.
// All auth logic preserved (signup, signin, Google, forgot
...
import { Spinner, Modal, ModalFooter, ModalHeader, ModalContent, ModalBody } from "@nextui-org/react";
...
function Login() {
  ...
  function validateIndianPhoneNumber(phone) { return /^(?:\+91)?[6-9]\d{9}$/.test(phone); }
  ...
  async function handleGoogleSignIn() {
    ...
    if (error) { toast.error("Google Sign-In failed. Please try again."); setGoogleLoading(false); }
  }

  // ── Auth state change ──
  useEffect(() => {
    const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === "PASSWORD_RECOVERY") { setPasswordModal(true); return; }
  ...
  async function handleSignUp() {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: { data: { full_name: formData.fullname, city: formData.city, phone: formData.phone, role: "user" } },
    });

    if (data) {
      if (data?.user?.email_confirmed_at != undefined) {
        toast.success("Account created. You can sign in now.");
        setLoading(false); toast.remove(r); setIsSignUp(false);
        return null;
      }
  ...
  async function forgotPassword(a) {
    ...
    try {
      ...
      toast.remove(r);
      if (res.data.success) { toast.success("Reset link sent. Check inbox and spam folder."); setFPModal(false); }
      else toast.error(res.data.message || "Unable to send reset link");
    } catch (err) { toast.remove(r); toast.error("Unable to send reset link. Please try again."); }
  }

  async function updatePassword(a) {
    if (a == undefined || a?.length < 8) { toast.error("Password must be at least 8 characters"); return null; }
          ...
          <ModalContent>
            {(onClose) => (
              <>
                ...
                <ModalFooter>
                  <button className={styles.googleBtn} style={{ width: "auto", padding: "0 20px", height: 42 }} onClick={() => setPasswordModal(false)}>Cancel</button>
          ...
          <ModalContent>
            {(onClose) => (
              <>
                ...
                <ModalFooter>
                  <button className={styles.googleBtn} style={{ width: "auto", padding: "0 20px", height: 42 }} onClick={() => setFPModal(false)}>Cancel</button>
        ...
        <div className={styles.wrap}>
          <div className={`${styles.card} ${!isSignUp ? styles.cardCompact : ""}`}>
            ...
            <div className={styles.formArea + " " + (isChanging ? styles.formHidden : "")}>
              <div className={styles.eyebrow}>{isSignUp ? "Get started" : "Sign in"}</div>
              ...
              <p className={styles.sub}>
                {isSignUp
                  ? "Join thousands preparing for IPMAT with India's #1 coaching."
                  : "Continue to your IPM Careers study panel."}
              </p>
              ...
              <div className={styles.form}>
                {isSignUp && (
                  <div className={styles.field}>
                    ...
                    <input
                      name="name" autoComplete="name" className={styles.input}
                      placeholder="Riya Sharma" type="text"
                ...
                {isSignUp && (
                  <div className={styles.field}>
                    ...
                    <input
                      name="phone" autoComplete="tel" className={styles.input}
                ...
                {isSignUp && (
                  <div className={styles.field}>
                    ...
                    <input
                      name="city" autoComplete="address-level2" className={styles.input}
