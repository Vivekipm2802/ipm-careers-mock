...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const module of modules || []) {
      ...
      if (countError) {
        console.error(`Error counting questions for module ${module.id}:`, countError);
        continue;
      }

      const subjectInfo = subjectMap[module.subject] || { title: "Unknown Subject" };

      topicsWithCounts.push({
        moduleId: module.id,
        title: module.title,
        subjectId: module.subject,
