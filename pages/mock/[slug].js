...
import { Avatar, Button,  Divider,  Input, Modal, ModalBody, ModalContent, ModalFooter, ModalHeader, Radio, RadioGroup, ScrollShadow, Spacer } from '@nextui-org/react';
      ...
      {question && question?.questionimage != undefined ? <img className='max-h-[30vh]' src={question?.questionimage}/>:''}
        <ul className='p-0'>
            ...
            <Radio className='flex flex-row items-center justify-start' value={index+1} key={index} >
              ...
              {option?.image != undefined ? <img src={option?.image} className='w-auto h-full max-h-12 object-contain'/> :<><p className='text-sm'>{option.title}</p> {process.env.NODE_ENV == "development" && option.isCorrect ? <div className='rounded-full w-1 h-1 bg-green-500'></div>:''}</>}
...
const {remainingTime} = useCountdown({duration:t,key:"fejak",isPlaying:is_allowed == false,colors:"#abc"})
...
const handleComplete = () => {
  if (currentSection === sections.length - 1 || config?.config?.switch_section == true) {
    setGameState(2)
...
async function getPlays(){
  ...
  const {data,error} = await supabase.from('mock_plays').select('id,created_at,test_id').eq('user',userDetails?.email).eq('test_id',alas.id);
  if(data){
if(data?.length > 0 )
  setExists(true)
  }
...
async function submitScore(a,b){
  ...
  setLoading(true)
  const {data,error} = await supabase.from('mock_plays').insert({
test_id:config?.id,
status:'completed',
report:a,
data:b
...
<Button color='primary' size='sm' as={Link} href={`/login?redirectTo=${router.asPath}`}>Login or Create an Account</Button>
</div>

    </div>
}
if(config?.config?.allow_retests == false && exists != undefined && exists == true && process.env.NODE_ENV != "development"  ){
  return <div className='flex flex-col relative justify-center align-middle items-center text-center font-sans h-screen w-full'>
<div className='flex flex-col justify-center items-center'>
You cannot reattempt this test.<br/> Please contact admin if you need help.
<Spacer y={2} x={2}></Spacer>
<Button color='primary' size='sm' as={Link} href={`/`}>Back to Dashboard</Button>
</div>
...
return <div className='flex flex-col w-full h-screen fixed left-0 top-0 justify-start  md:justify-center items-center p-0 md:p-8 bg-gray-50'>
<div className='w-full max-w-[800px]'>
<div className='w-full h-auto overflow-hidden rounded-none md:rounded-xl shadow-md border-1 border-gray-300'><img className='w-full h-full object-cover' src={data?.image}/></div>
...
<p className='p-2 rounded-xl mt-4 bg-red-50 border-1 border-red-300 text-red-600'>Test Expired</p>
</>: <>
<p className='text-sm my-2'>Test is not unlocked yet ,<br/> Please check again later after the test start time</p>
...
:''}
     ...
     <div className='text-xs font-sans overflow-y-auto max-h-[70vh]' dangerouslySetInnerHTML={{__html:config?.config?.instructions}}></div></>:''}
     {insindex == 1 ? <>
      <div className='text-xs font-sans overflow-y-auto max-h-[70vh]' dangerouslySetInnerHTML={{__html:config?.config?.instructions2}}></div>
     </>:""}
     </div>:''}     
     {gamestate == 1 ? <>
        ...
        <ScrollShadow className='w-full flex flex-col h-full justify-start align-start items-start overflow-y-auto'>
          ...
          <QuestionCard index={questions[currentQ]?.seq} onSelect={(e)=>addItem({...e,at:timeDuration - totalSeconds})} answered={answered}  key={'ipmc'+currentQ}  question={questions[currentQ]}  />
...
<h2 className='text-2xl text-center text-primary px-6 w-full'>Your Responses have been submitted and now you are being redirected to results page.</h2>
...
<div className='p-4 rounded-xl border-1 border-gray-200 flex flex-row'>
  ...
  <div className='flex flex-col p-2 px-4'>
    <h2 className='text-xl font-bold text-primary'>{userDetails?.user_metadata?.full_name}</h2>
  </div>
  </div></div>
:''}
...
const {data,error} = await serversupabase.from('mock_test').select('*').eq('uid',context.query.slug)
...
function getStatus(givenStartTime, givenEndTime) {
  ...
  return {props:{config:data[0],is_allowed:(data[0]?.start_time || data[0]?.end_time)  && p == false  ? getStatus(data[0]?.start_time,data[0]?.end_time):true,data:data[0]}}
}
