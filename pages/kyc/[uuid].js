...
import { Button, Dropdown, DropdownItem, DropdownMenu, DropdownTrigger, Input, Modal, ModalBody, ModalContent, ModalFooter, ModalHeader, Radio, RadioGroup, Spacer, modal } from '@nextui-org/react';
...
if(questions == undefined || questions?.length == 0 ){
  return <></>
    ...
    return <div className='hidden lg:flex flex-col'> 
    <div className='flex flex-row py-2'>
{questions && questions?.slice(0,questions?.length/2).map((i,d)=>{
        return <div className={`h-[18px] w-[18px] md:md-2 md:h-[14px] md:w-[14px] relative transition-all bg-gray-300 rounded-full mx-1 ${d < level ? "bg-primary":'scale-75'}`}>
...
{d < level -1 ? <div className='absolute top-[50%] right-[-100%] translate-y-[-50%] w-full bg-primary h-[1px]'></div>:''}
    ...
    <div className='flex flex-row py-2'>
{questions && questions?.slice(questions?.length/2 + 1,questions?.length).map((i,d)=>{
        return <div className={`h-[18px] w-[18px] md:md-2 md:h-[14px] md:w-[14px] relative transition-all bg-gray-300 rounded-full mx-1 ${d + questions?.length/2 < level ? "bg-primary":'scale-75'}`}>

{d + questions?.length/2 < level - 1 ? <div className='absolute top-[50%] right-[-100%] translate-y-[-50%] w-full bg-primary h-[1px]'></div>:''}
        </div>
    })}
...
const QuestionCard = ({ question, handleOptionClick,time,handleSubmitAnswer,isPlaying,endGame }) => {
    const [answer,setAnswer] = useState();
if(question == undefined){
  return <div>Question Undefined {console.log('Question is:'+question)}</div>
    ...
    return (
      <div className='sf w-full max-w-[800px] h-full overflow-y-auto mx-auto p-5 flex-1 justify-center align-middle items-start flex flex-col text-left '>
        <div className='shadow-lg rounded-xl w-full p-4 lg:p-10 relative'>
            <div className='absolute top-5 right-5'><CountdownCircleTimer key={1} isPlaying={true} size={46} strokeWidth={3}
    duration={4200}
    colors={['#05ffa3', '#fbff05', '#ffa305', '#ff0505']}
    colorsTime={[90, 60, 30, 0]}
     onComplete={()=>{endGame()}} >
        {({ remainingTime }) => <p className='font-bold flex flex-row text-xs'>{Math.floor(remainingTime/60)}:{remainingTime % 60}</p>}
        </CountdownCircleTimer></div>
        <h2 className='font-bold text-2xl text-primary'>{question.title}</h2>
        <div className='font-light max-h-[50vh] overflow-y-auto text-sm qcontent' dangerouslySetInnerHTML={{__html:question.question}}></div>
      ...
      {question && question?.questionimage != undefined ? <img src={question?.questionimage}/>:''}
        <ul className='p-0'>
          ...
          {question.options.map((option, index) => (
            <li onClick={() => handleOptionClick(option.isCorrect, option.text,option.text,question,option.popupimage)} key={index} className="rounded-xl flex flex-row items-center justify-center shadow-md p-3 m-2 mt-2 hover:bg-primary hover:text-white  text-xs md:text-sm cursor-pointer transition-all">
              
              {option.image ? <img src={option.image} className=' object-contain w-auto max-w-[80px]'/>:option.title}
...
if(level != undefined && questions && !completed.includes(questions[level].parent)){
...
const {data,error} = await supabase.from('results').select("id").match({'email':a,'status':'finished',test:test_id});
...
toast.error('This test already exists in attempted test , continue the test from dashboard or contact admin.');
toast.remove(r)
...
async function getPsuedoModule(a){
  const {data,error} = await supabase.rpc('get_psuedo',{test_id_param:test_id})
  if(data){
setPsuedo(data)
  }else{}
}
...
async function getQuestions(){
    ...
    const {data,error} = await supabase.rpc('get_swot_questions_by_parent',{parent_id:test_id})
if(data){
...
const configdata = Array.from(scoredata).map((i,d)=>{
  ...
  return {module:i , score : 0 , total : testData.filter(item=>item.parent_id == i)[0]?.match_count }
});
...
/* console.log(dataToInsert) */
  const {data,error} = await supabase.from('scores').insert([...dataToInsert]).select();
  if(data){
...
const {data,error} = await supabase.from('results').update({status:"finished"}).eq('id',a).select();
if(data){
...
console.log(moduleIndex)
  setScoreboard(prevData => {
    ...
    newData[moduleIndex] = { ...newData[moduleIndex], score: newData[moduleIndex].score + 1 }; // Update the value at the given index
  ...
  const handleOptionClick = (isCorrect, congratstext,headertext,cQ,popup) => {
...
updateModuleScore(1,cQ.parent)
    ...
    setSecondaryLevel(secondaryLevel + 1);
      ...
      setModalContent({ color: 'green', text: congratstext,message: headertext, isCorrect:true,popupimage:popup });
      fire();
      setShowModal(true);
      generateRandomKey()
    } else {
      ...
      setModalContent({ color: 'red', text: congratstext , message: headertext, isCorrect:false,popupimage:popup});
      setShowModal(true);
      generateRandomKey()
    }
  };
...
<Modal placement="center" scrollBehavior="inside" className='sf overflow-hidden' isOpen={insModal} backdrop='opaque'  classNames={{backdrop:"opacity-10 bg-overlay/5"}}  isDismissable={false} >
<ModalContent>
{(onClose)=>(<> 
<ModalHeader>

<h2 className='text-secondary'>
Self-Assessment SWOT Test</h2>
...
<br/>
This quick test will help you gauge your strengths and weaknesses. It will adapt to your responses, providing a personalized assessment experience.

<br/><br/>
<strong className='text-black'>Instructions:</strong>
<br/><br/>

Take your time to read each question and response options carefully. <br/><br/>
Avoid underestimating or overestimating your abilities. Your goal is to assess your current level of understanding accurately.
Once you have answered all the questions, review your responses, and ensure you have not missed any questions. <br/><br/>
Good luck! Your honest self-assessment will help tailor the course to your needs. Begin the test when you're ready and give your best. <br/><br/>
<strong className='text-black'>Please Remember</strong> – The test cannot be paused and needs to be completed in one ‘Go.’
...
<ModalFooter>
  {consent == false ? 
<Button color='danger' variant="ghost" onPress={()=>{router.back()}}>Cancel</Button>:''}
  <Button color='primary' className='text-white' onPress={()=>{handleStart()}}>Continue</Button>
</ModalFooter>
</>)}</ModalContent> </Modal>
...
{uniqueSubjects && uniqueSubjects.map((i,d)=>{

  return <div className='bg-white flex-1 p-0'>
    
    <h2 className='w-full bg-primary rounded-xl text-white p-3'>{i}</h2>
    <div className='w-full flex flex-row flex-wrap justify-center items-center my-4'>
    {testData && testData.filter(item => item.subject_name == i).map((z,v)=>{
      return <div className='p-2 flex flex-row justify-center bg-white rounded-full shadow-md mb-1 mr-1 px-4 text-sm border-1 border-gray-50 align-middle items-center '>{z?.parent_title} <div className='bg-primary ml-2 text-white rounded-full w-auto px-2 h-auto flex justify-start items-start align-middle text-center text-xs relative'><p className=''>{z.match_count} Questions</p></div></div>
    })}</div>
    </div>
})}</div>
<div dangerouslySetInnerHTML={{__html:parentData?.parent?.objective}}></div>
        <div className='mt-2 '>
            <Button color='default' className='mr-2 px-5 sf' onClick={()=>{router.back()}}>Go Back</Button>
            <Button color='primary' className='px-5 sf text-white to-primary border-1 shadow-sm shadow-primary from-primary-900 bg-gradient-to-tr' onClick={()=>{prepareGame()}}>{isContinue == null || isContinue == undefined  ? 'Start':'Continue'} SWOT Test</Button>
        </div>
        </div>
...
<div className='border-1 rounded-xl text-sm border-white p-1 px-2 w-auto'>
   ...
   {modalContent && modalContent.isCorrect == false ? <p>You have not scored any Point</p>:<p>You have earned {config.increment} Point</p> }</div>
    </ModalHeader>
    <ModalBody>
    
    {modalContent?.popupimage != undefined ? <img className='max-w-[400px] rounded-lg overflow-hidden shadow-md object-cover' src={modalContent?.popupimage}/>:<div dangerouslySetInnerHTML={{__html:modalContent?.text}}></div>}
    ...
    <Button className='flex-1 text-white' color='primary' onPress={()=>{setShowModal(false)}}>Next</Button>
...
<h2 className='text-md font-bold'>
          ...
          {testData && testData.map((i,d)=>{
            return <div className={'w-full bg-white border-1 flex flex-row items-center align-middle justify-center border-gray-200 my-1 rounded-lg p-2 text-center text-sm' + ` ${i.parent_id == questions[level].parent  ? "!bg-green-200 border-green-400" : " "}`} data-status={testData && testData?.filter(item => item.parent_id == questions[level].parent).length > 0  ? true : false} >
             
             
              {i.parent_title}
              {completed.includes(i.parent_id) ? <svg className='ml-auto' width="24" height="24" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12 6.477 2 12 2Zm3.22 6.97-4.47 4.47-1.97-1.97a.75.75 0 0 0-1.06 1.06l2.5 2.5a.75.75 0 0 0 1.06 0l5-5a.75.75 0 1 0-1.06-1.06Z" fill="#2ECC70"/></svg>:''}
              </div>
          })}</div>
        </div>
        <div className='flex-1 flex flex-col justify-center align-middle items-stretch h-full'>
          <div className='flex flex-col lg:flex-col font-bold text-sm lg:text-sm gap-2 mt-2 lg:mt-0 md:mt-0 p-2  text-left justify-start align-middle items-start w-full'>
            ...
            <p>Question Module: {testData.filter(item => item.parent_id == questions[level]?.parent)[0]?.parent_title || "Not Found"}</p>
         ...
         {/*  <p className='flex flex-row justify-center align-middle items-center'>Lives: <span className='flex flex-row'>{lives != undefined && Array(lives).fill().map((i,d)=>{
          ...
          <div className='absolute top-5 right-5'>
            <h2 className='text-right'>Level : {level + 1}/{questions?.length}</h2>
...
<DropdownMenu>
    <DropdownItem>
        <div dangerouslySetInnerHTML={{__html:questions[level].hint}}></div>
          ...
          <QuestionCard endGame={()=>{setLevel(questions?.length)}} isPlaying={!(showModal || isHintVisible)} key={1} time={parentData?.parent?.time} handleSubmitAnswer={(e,f,g,h)=>{console.log(e,f,g,h),handleSubmitAnswer(e,f,g,h)}} question={questions[level]} handleOptionClick={handleOptionClick} />
...
<p className='font-bold text-green-600 text-2xl text-center'>You have successfully completed {parentData?.parent?.title}</p>
...
<Button className='my-2 text-white' color='secondary' onPress={()=>{router.back()}}>View your Result</Button>
</div>
:''}
    </div> 
...
const {data,error} = await serversupabase.from('swot_test').select('id,course(title)').eq('uid',uuid)
if(data && data?.length > 0){
...
return {props:{test_id:data[0]?.id , isContinue : continue_id ||  null , course : data[0]?.course?.title || "Not Found"}
