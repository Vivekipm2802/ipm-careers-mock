...
export function getStatusIcon(a,report,isSpecial){
    
    if(report?.some(item=>item.id == a.id && item.status == "markedForReview")){
      return <img className=' z-0 absolute left-0 top-0 w-full h-full object-contain' src='/amr.svg'/>
    }
    if(report?.find(item=>item.id == a.id) && ((isSpecial == true && report?.find(item=>item.id == a.id)?.isCorrect == false) || false)){
      return <img className=' -z-[10] absolute left-0 top-0 w-full h-full object-contain' src='/na.svg'/>
    }
  
    if(report?.find(item=>item.id == a.id)){
      return <img className=' z-0 absolute left-0 top-0 w-full h-full object-contain' src='/sc.svg'/>
    }
    if(report?.filter(item=>item.status == "review")?.some(item=>item.id == a.id)){
...
export default function QuestionBrowser  ({sideBarActive,setSidebarActive,gamestate,questions,report}){

    const {userDetails} = useNMNContext()
    function getStatus(a){
    
      if(report?.some(item=>item.id == a.id) && report?.filter(item=>item.status == "review").some(item=>item.id == a.id)){
      ...
      return <div className={'flex h-full flex-col w-full max-w-0 bg-white shadow-[-2px_-2px_12px_-6px_#6663] transition-all z-[20] ease-in-out duration-300 translate-x-full fixed right-0 top-0  lg:relative  lg:translate-x-0 ' + (sideBarActive ? ' !max-w-[400px] !translate-x-0':'') }>
    ...
    <div className='p-4 font-sans flex flex-row flex-wrap text-xs w-full'>
      ...
      <div className='w-8 h-8 relative'><img className='w-full h-full object-contain z-0' src='/sc.svg'/>
        ...
        <p className='text-white flex text-center'>{report?.length || 0}</p></div>
        </div>
    ...
    {/*   <div className=' w-1/2 flex-row flex items-center justify-start p-1'>
      ...
      <div className='w-8 h-8 relative'><div className='w-8 h-8 object-contain border-1 text-black border-gray-400 aspect-square flex flex-col items-center justify-center rounded-md from-white to-gray-200 bg-gradient-to-b' />
        <div className='absolute left-0 top-0 w-full h-full flex flex-col z-10 justify-center items-center mt-[1px]'>
        <p className='text-black flex text-center'>{questions?.length - (report?.length || 0)}</p></div>
        </div>
      ...
      <div className='w-8 h-8 relative'><img className='w-full h-full object-contain z-0' src='/mr.svg'/>
        <div className='absolute left-0 top-0 w-full h-full flex flex-col z-10 justify-center items-center mt-[1px]'>
        <p className='text-white flex text-center'>{report?.filter(item=>item.status =="review")?.length || 0}</p></div>
        </div>
        <Spacer x={2} y={2}></Spacer>
      ...
      <div className=' w-full flex-row flex items-center justify-start p-1 relative'>
        ...
        <div className='absolute left-0 top-0 w-full h-full flex flex-col z-10 justify-center items-center mt-[1px]'>
        <p className='text-white flex text-center'>{report?.filter(item => item.status == "markedForReview")?.length||0}</p></div>
        </div>
        <Spacer x={2} y={2}></Spacer>
    ...
    <div className='flex flex-row relative items-center justify-start flex-wrap'>
      {questions && questions.map((i,d)=>{
        return <div className='p-1 relative group' onClick={()=>{toast.error('You cannot switch question in this test.') }}>
