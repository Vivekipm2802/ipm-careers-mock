...
const Game = ({test_data,questions}) => {
  ...
  const handleComplete = () => {
    setGameState(2)
    toast.success('Test is only submitted in paid courses')
  };
...
const router = useRouter();
  ...
  async function getUserStatus(){
     ...
     const {data} = await supabase.rpc('is_user_paid');
...
if(status == true) 
{
return <div className='flex flex-col items-center justify-center text-sm h-screen w-full'>Demo Level not allowed for paid user
...
export async function getServerSideProps(context) {
  // Fetch the test data first
  const { data: testData, error: testError } = await serversupabase
    .from('self_learning_tests')
    .select('*,level_id(*),vuid(*),luid(*)')
    .eq('uuid', context.query.uuid);

  // Check for test data or redirect to 404 if not found
  ...
  const test_data = testData[0];

  
  // Now fetch levels data using the resolved test_data.level_id
  const { data: levelData, error: levelError } = await serversupabase
    .from('levels')
    .select('*,questions!questions_parent_fkey(*)')
    .eq('id', test_data?.level_id?.id)
  ...
  if (levelData && levelData.length > 0) {
    questions = levelData[0]?.questions || [];
    parentData = { ...levelData[0] };
    delete parentData["questions"]; // Remove questions from parent data
  }
